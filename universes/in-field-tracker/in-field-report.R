...
sum(coalesce(number_of_voters,0)) over (partition BY state_code||list_source) as number_of_voters_in_ventile,
coalesce(activists,0) as activists,
sum(coalesce(activists,0)) over (partition BY state_code||list_source) as activists_in_ventile,
...
collected_after_list_pass,
coalesce(number_of_voters,0) as number_of_voters,
sum(coalesce(number_of_voters,0)) over (partition BY state_code) as number_of_voters_in_ventile,
coalesce(activists,0) as activists,
sum(coalesce(activists,0)) over (partition BY state_code) as activists_in_ventile,
round(coalesce(1.0*ccj_1/nullif(ccj_all,0),0),4) as ccj_1rate,
coalesce(ccj_1,0) as ccj_1,
coalesce(ccj_2,0) as ccj_2,
coalesce(ccj_3,0) as ccj_3,
