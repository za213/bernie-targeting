CREATE TABLE bernie_elemmerman.event_invite_table
distkey(person_id) 
sortkey(person_id) as
(SELECT *
        ,NTILE(20) OVER (PARTITION BY state_code ORDER BY event_invite_rank ASC) AS event_invite_20
 from
 (select  person_id
         ,state_code
         ,case 
...
where (civis_2020_partisanship >= .66 or party_8way = '1 - Democratic' or any_activist_donor_flag = 1 or any_activist_donor_household_flag = 1)
