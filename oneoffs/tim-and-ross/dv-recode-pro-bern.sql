DROP TABLE IF EXISTS bernie_nmarchio2.nh_survey_dvs;
CREATE TABLE bernie_nmarchio2.nh_survey_dvs
  DISTSTYLE KEY
  DISTKEY (person_id)
  SORTKEY (person_id)
AS 
...
DROP TABLE IF EXISTS bernie_nmarchio2.nh_field_dvs;
CREATE TABLE bernie_nmarchio2.nh_field_dvs
  DISTSTYLE KEY
  DISTKEY (person_id)
  SORTKEY (person_id)
AS 
(select * from (SELECT person_id::varchar(10) ,
       CASE
           ...
           ELSE NULL END AS pro_bernie_id_1_no_gop ,
       CASE
           WHEN latestsupportid = 1 THEN 1
           WHEN latestsupportid IN (2,3,4,5) THEN 0
           ELSE NULL END AS pro_bernie_id_1 
FROM bernie_nh.dim_voters) where pro_bernie_id_1 is not null and pro_bernie_id_1 is not null);
